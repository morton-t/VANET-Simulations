...
protected:
    ...
    std::string formatCoords(double x, double y);

    void handleBroadcast(Broadcast* wsm);
    void handleChallenge(ChallengePacket* wsm);
    void handleResponse(ResponsePacket* wsm);

    void createChallenge(std::string recipient);
};
