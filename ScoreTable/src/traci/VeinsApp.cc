...
void VeinsApp::onWSM(BaseFrame1609_4* msg)
{
    ...
    int trustModifier = 0;
    //check if vehicle is in suspected vehicles
    for (int i = 0; i < wsm->getSuspectedGhostsArraySize(); ++i) {                                      //FIXME: This does not correctly check the suspectedGhosts
        std::cout << "\t" << incomingVehID.c_str() << " " << wsm->getSuspectedGhosts(i) << std::endl;   //       array in the VeinsApp class. This needs to append the
        if (incomingVehID.c_str() == wsm->getSuspectedGhosts(i)) {                                      //       suspect vehicles in the wsm to the vector and then
            trustModifier = -1;                                                                         //       cehck if the incoming vehicle ID is in this array
            std::cout << "Suspicious vehicle found!" << std::endl;
            break;
        }
    }
    ...
    if (incomingVehTableData[0] == "null") {
        incomingVehTableData = {incomingVehID, incomingVehSpeed, incomingTimeStamp, incomingVehType, "passenger", std::to_string(0 + trustModifier)};
    }
    ...
    //Update vector of suspect ghosts
    suspectedGhosts = st.suspectedGhosts;
...
/*
    ...
    if (incomingVehTableData[0] != "null") {
        incomingTrustScore = std::stoi(incomingVehTableData[VEH_TRUSTSCORE_INDEX]);

        //If the vehicle is flagged as a ghost, do not update its score table data
        if (std::stoi(incomingVehTableData[VEH_TRUSTSCORE_INDEX]()) <= MIN_TRUST_THRESHOLD) { return; }
    }
    else {
...
void VeinsApp::handleSelfMsg(cMessage* msg)
{
    ...
    if (selfTableData[0] == "null") {
        std::array<std::string, DATAPOINTS_PER_ROW> newEntry = {vehID, vehSpeed, timestamp, vehType, "self", "0"};
        st.setScoreTable(newEntry);
    }
    else {
        std::array<std::string, DATAPOINTS_PER_ROW> newEntry = {vehID, vehSpeed, timestamp, vehType, "self", selfTableData[VEH_TRUSTSCORE_INDEX]};
        st.setScoreTable(newEntry);
    }
...
void VeinsApp::handlePositionUpdate(cObject* obj)
{
    ...
    //
    wsm->setSuspectedGhostsArraySize(suspectedGhosts.size());
    for (int i = 0; i < suspectedGhosts.size(); ++i) {
        wsm->setSuspectedGhosts(i, suspectedGhosts.at(i).c_str());
    }
